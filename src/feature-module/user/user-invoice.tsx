...
const UserInvoice = () => {
        ...
        backendFunctions.transactions
            ...
            .then((response: tAllTransactions) => {
                ...
                setTransactionsToDisplay(
                    response.filter(
                        (transaction: tTransaction) => transaction.clientId === isUserConnected.id
                    )
                );
            })
            .catch((error) => {
    ...
    const Render = (res: tTransaction) => {
        // Get styling information from the utility function
        const statusInfo = getPaymentStatus(res.status);

        return (
            <td className="table-accept-btn text-end">
                <Link to="#" className={`badge ${statusInfo.bg}`}>
                    <i className={`feather-${statusInfo.icon} me-1`}></i>
                    {statusInfo.label}
                </Link>
            </td>
        );
