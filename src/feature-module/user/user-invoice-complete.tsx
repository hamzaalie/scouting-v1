...
import {allCourt, idaSportsUserInterface} from "../../core/data/interface/model";
...
import {formatPrice, options} from "../../helpers/input.helper";
import {LoadingIcon} from "yet-another-react-lightbox";

const UserInvoiceComplete = () => {
    ...
    const [allTransactions, setAllTransactions] = useState<tAllTransactions>([]);
    const [transactionsToDisplay, setTransactionsToDisplay] = useState<tAllTransactions>([]);
    const [loadingData, setLoadingData] = useState<boolean>(true);

    useEffect(() => {
        //Query
        backendFunctions.transactions
            .getAllTransactions(localStorageFunctions.getToken())
            .then((response: tAllTransactions) => {
                ...
                setTransactionsToDisplay(
                    response.filter(
                        (transaction: tTransaction) =>
                            transaction.clientId === isUserConnected.id &&
                            successStatus.includes(transaction.status)
                    )
                );
            })
            .catch((error) => {
    ...
    const filteredData = transactionsToDisplay.filter((datas) => {
    ...
    const courtNameRender = (res: tTransaction) => {
        return (
            <h2 className="table-avatar">
                ...
                <span className="table-head-name flex-grow-1">
                    {/* //TODO Ask to retrieve pack info from packId  */}
                    <Link to="#">{res.ref}</Link>
    ...
    const amountRender = (res: tTransaction) => {
        //TODO Ask to add res.currency for the transaction. For now, default is FCFA
        return <h4 className="text-center">{formatPrice("fr-FR", res?.amount, "CFA")}</h4>;
    };
    const actionRender = () => {
        return (
    ...
    const dateTimeRender = (res: tTransaction) => {
        return (
            <h4>
        ...
        return (
            <td className="table-accept-btn text-end">
                ...
                >
                    <i
                        className={
                            successStatus.includes(res.status)
                                ? "feather-check-circle me-1"
                                : "feather-x-circle me-1"
                        }
                    ></i>
                    {["succès", "ACCEPTED", "Accepted", "INITIALIZED", "Initialized"].includes(
                        res.status
                    )
                        ? "Succès"
                        : "Échec"}
    ...
    const paymentRender = (res: tTransaction) => {
        return (
            <td className="table-accept-btn text-end">
                <Link to="#" className={"badge " + getPaymentMode(res.paymentMode).badge}>
                    <i></i>
                    {getPaymentMode(res.paymentMode).label}
                </Link>
            </td>
        );
    };
                        ...
                        <div className="col-lg-12">
                            <div className="dashboard-menu">
                                <ul>
                                    ...
                                    <li>
                                        <Link to={routes.userBookings} className="">
                                            <ImageWithBasePath
                                    ...
                                    <div className="row d-flex align-items-center">
                                        ...
                                        <div className="col-xl-12 col-lg-12 col-sm-12 col-12">
                                            <div className="coach-court-list">
                                                <ul className="nav">
                                                    <li>
                                                        <Link to={routes.userInvoice}>Tous</Link>
                                                    </li>
                                                    <li>
                                                        <Link
                                                            to={routes.userInvoiceComplete}
                                                            className="active"
                                                        >
                                                            Succès
                                                        </Link>
                                                    ...
                                                    <li>
                                                        <Link to={routes.userInvoiceCancelled}>
                                                            Échec
                                                        </Link>
                                                    </li>
                                                </ul>
                                            ...
                                            >
                                                <div className="table-responsive table-datatble">
                                                    ...
                                                    >
                                                        <Column
                                                            sortable
                                                            field="ref" //TODO Put packName here
...
export default UserInvoiceComplete;
